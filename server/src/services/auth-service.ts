...
export async function forgotPassword(params: { email: string }): Promise<{ ok: true; resetUrl?: string }> {
  const env = getEnv()
  const email = params.email.trim().toLowerCase()
...
export async function resetPassword(params: { token: string; password: string }): Promise<{ ok: true }> {
  const env = getEnv()
...
function signToken(userId: string, env: ReturnType<typeof getEnv>): string {
