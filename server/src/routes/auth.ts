...
export function errorToHttp(err: unknown): { status: number; message: string } {
  if (err instanceof AuthError) return { status: err.status, message: err.message }
  if (err instanceof z.ZodError) return { status: 400, message: err.issues[0]?.message ?? 'Invalid request' }
